...
def route_to_tail_support(test_cls: torch.Tensor, memory_bank: Dict[int, Dict]) -> Dict:
    ...
    support_cls = torch.stack([memory_bank[support_id]['support_cls'] for support_id in support_ids], dim=0).to(test_cls.device)
    support_cls = _normalize_rows(support_cls)
    similarity = support_cls @ test_cls
    ...
    adaptive_angle = float(memory_bank[best_sample_idx]['adaptive_angle'])
    mem_applied = int(best_angle <= adaptive_angle)
    return {
        'mem_applied': mem_applied,
        'routed_sample_idx': best_sample_idx,
        'routed_sample_key': int(memory_bank[best_sample_idx].get('sample_key', best_sample_idx)),
...
def compute_tail_support_memory_scores_for_batch(model,
    ...
    for index in range(images.shape[0]):
        route = route_to_tail_support(cls_embeddings[index], memory_bank)
