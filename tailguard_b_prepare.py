...
from tailguard_b_artifacts import save_tailguard_b_prepare_artifacts
...
def _compute_adaptive_angles(features: torch.Tensor, args):
    default_angle = float(getattr(args, 'tgb_default_adaptive_angle', 5.0))
...
def _build_head_group_assignments(group_embeddings: np.ndarray, metadata_df: pd.DataFrame, candidates_df: pd.DataFrame, args):
    ...
    if int(head_mask.sum()) == 0:
        raise ValueError('TailGuard-B prepare requires at least one non-tail H sample')
...
@torch.no_grad()
def prepare_tailguard_b_metadata(model,
    ...
    if len(metadata_df) == 0:
        raise ValueError('TailGuard-B prepare requires non-empty train metadata')
    ...
    if group_embedding_source == tail_embedding_source:
        ...
        if group_metadata_df['sample_idx'].astype(int).tolist() != metadata_df['sample_idx'].astype(int).tolist():
            raise ValueError('group embedding metadata mismatch during TailGuard-B prepare')
    ...
    if cls_metadata_df['sample_idx'].astype(int).tolist() != train_metadata_df['sample_idx'].astype(int).tolist():
        raise ValueError('cls embedding metadata mismatch during TailGuard-B prepare')
    ...
    if output_dir is not None:
        ...
        saved = save_tailguard_b_prepare_artifacts(
            output_dir,
            candidates_df,
            train_metadata_df,
            head_group_assignments_df,
