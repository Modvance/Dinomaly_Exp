"""TGM runner retained for historical ablations of the pre-mainline method."""
...
from tgm_artifacts import (
    save_tgm_iteration_artifacts,
    save_tgm_memory_artifacts,
    save_tgm_prune_artifacts,
    save_tgm_summary,
)
from tgm_gbps import (
    aggregate_tgm_risk_history,
    build_tgm_delete_plan,
    run_tgm_gbps_iteration,
)
from tgm_memory import build_tail_group_memory_bank, run_tail_group_memory_evaluation
from tgm_prepare import prepare_tgm_training_metadata
...
def train(item_list):
    ...
    if contaminated_paths is None:
        print_fn('tgm diagnosis: contamination manifest not found, contamination-aware logging will be unavailable.')
    else:
        print_fn('tgm diagnosis: loaded contamination manifest {}.'.format(manifest_path))
    ...
    prepare_output_dir = os.path.join(args.save_dir, args.save_name, 'tgm_prepare')
    prepare_result = prepare_tgm_training_metadata(
        model,
    ...
    print_fn('tgm prepare: tail_candidates={}, groups={}'.format(
    ...
    for _ in range(int(np.ceil(total_iters / len(train_dataloader)))):
        ...
        for batch in train_dataloader:
            ...
            if gbps_trigger_iter is None and should_run_check(
                ...
                tgm_result = run_tgm_gbps_iteration(
                    scored_df.copy(),
                ...
                trigger_result = evaluate_gbps_ci_peak_trigger(
                    U_t=tgm_result['gbps_U'],
                    SE_t=tgm_result['gbps_SE'],
                    noise_evidence=tgm_result['global_noise_evidence'],
                    best_U=gbps_best_U,
                ...
                summary = dict(tgm_result['summary'])
                summary.update(trigger_result['summary'])
                summary['tgm_prepare_summary'] = prepare_result['summary']
                ...
                save_gbps_artifacts(
                    iter_dir,
                    group_metrics_df=tgm_result['group_metrics_df'],
                    bootstrap_df=tgm_result['bootstrap_df'],
                    sample_group_scores_df=tgm_result['sample_group_scores_df'],
                    extra_summary=summary,
                    scored_df=scored_df,
                )
                save_tgm_iteration_artifacts(
                    iter_dir,
                    tgm_result['global_group_metrics_df'],
                    tgm_result['sample_tgm_scores_df'],
                    extra_summary={
                        'tgm_local_risk_mean': tgm_result['summary']['tgm_local_risk_mean'],
                        'tgm_global_group_risk_mean': tgm_result['summary']['tgm_global_group_risk_mean'],
                        'tgm_final_risk_mean': tgm_result['summary']['tgm_final_risk_mean'],
                        'tgm_num_global_active_groups': tgm_result['summary']['tgm_num_global_active_groups'],
                    },
                )
                ...
                if trigger_result['triggered']:
                    ...
                    postprocess_summary = {
                        ...
                        'tgm_postprocess_mode': args.gbps_postprocess_mode,
                    ...
                    if args.gbps_postprocess_mode == 'remove' and not gbps_has_postprocessed and selected_scored_df is not None:
                        aggregated_risk_df = aggregate_tgm_risk_history(
                            args.diag_save_dir,
                            gbps_selected_iter,
                            lookback_checks=int(args.tgm_risk_lookback_checks),
                        )
                        train_image_count_before_prune = len(train_data)
                        delete_plan = build_tgm_delete_plan(
                            selected_scored_df,
                            prepare_result['group_assignments_df'],
                            prepare_result['train_metadata_df'],
                            aggregated_risk_df,
                        ...
                        postprocess_summary.update({
                            'tgm_pruned_count': int(delete_plan['summary']['num_pruned']),
                            'tgm_retained_count': int(delete_plan['summary']['num_retained']),
                            'tgm_tail_support_count': int(delete_plan['summary']['num_tail_support']),
                            'tgm_tail_suspicious_count': int(delete_plan['summary']['num_tail_suspicious']),
                            'train_image_count_before_prune': int(train_image_count_before_prune),
                            'train_image_count_after_prune': int(len(train_data)),
                        })
                        save_tgm_prune_artifacts(iter_dir, delete_plan, extra_summary=postprocess_summary)
                        print_fn('tgm remove iter {}: pruned {} / {} training samples, train image number {} -> {}'.format(
                            gbps_selected_iter,
                    ...
                    print_fn('tgm trigger iter {}: status={}, selected_iter={}'.format(
                        gbps_trigger_iter,
    ...
    memory_bank = {}
    if bool(args.tgm_memory_enable):
        ...
        memory_output_dir = os.path.join(args.save_dir, args.save_name, args.tgm_memory_artifact_dir)
        memory_metadata = {
            'checkpoint_path': args.checkpoint_path,
            'encoder_name': checkpoint_metadata['encoder_name'] if checkpoint_metadata is not None else args.encoder_name,
            'image_size': image_size,
            'crop_size': crop_size,
            'tgm_memory_groups': int(len(memory_bank)),
            'tgm_memory_support_samples': int(len(support_df)),
            'tgm_mem_min_support_images': int(args.tgm_mem_min_support_images),
            'tgm_mem_gate_quantile': float(args.tgm_mem_gate_quantile),
            'tgm_mem_gate_mad_scale': float(args.tgm_mem_gate_mad_scale),
            'tgm_mem_min_gate_angle': float(args.tgm_mem_min_gate_angle),
            'tgm_mem_fusion_lambda': float(args.tgm_mem_fusion_lambda),
            'tgm_mem_topk_ratio': float(args.tgm_mem_topk_ratio),
        }
        memory_saved = save_tgm_memory_artifacts(
            memory_output_dir,
            memory_bank,
            score_df,
            per_class_metrics,
    ...
    summary_path = save_tgm_summary(os.path.join(args.save_dir, args.save_name), tgm_summary)
    print_fn('saved tgm summary to {}'.format(summary_path))


if __name__ == '__main__':
    os.environ['CUDA_LAUNCH_BLOCKING'] = '1'

    parser = argparse.ArgumentParser(description='Standalone TGM runner for Dinomaly MVTec')
    parser.add_argument('--data_path', type=str, default='../mvtec_anomaly_detection')
    parser.add_argument('--save_dir', type=str, default='./saved_results')
    parser.add_argument('--save_name', type=str, default='vitill_mvtec_tgm')
    parser.add_argument('--diag_save_dir', type=str, default='tgm_diag')
    ...
    parser.add_argument('--tgm_tail_embedding_source', type=str, default='encoder_cls', choices=['encoder', 'encoder_cls'])
    parser.add_argument('--tgm_save_cls_embeddings', action='store_true')
    parser.add_argument('--tgm_save_tailsampler_analysis', dest='tgm_save_tailsampler_analysis', action='store_true', default=True)
    parser.add_argument('--no-tgm_save_tailsampler_analysis', dest='tgm_save_tailsampler_analysis', action='store_false')
    parser.add_argument('--tgm_save_tailsampler_analysis_details', dest='tgm_save_tailsampler_analysis_details', action='store_true', default=True)
    parser.add_argument('--no-tgm_save_tailsampler_analysis_details', dest='tgm_save_tailsampler_analysis_details', action='store_false')
    ...
    parser.add_argument('--tgm_enable_global_group_risk', dest='tgm_enable_global_group_risk', action='store_true', default=True)
    parser.add_argument('--no-tgm_enable_global_group_risk', dest='tgm_enable_global_group_risk', action='store_false')
    parser.add_argument('--tgm_global_tau_bic', type=float, default=0.0)
    parser.add_argument('--tgm_global_tau_sep', type=float, default=0.5)
    parser.add_argument('--tgm_global_tau_conf', type=float, default=0.1)
    parser.add_argument('--tgm_global_pi_min', type=float, default=0.05)
    parser.add_argument('--tgm_global_pi_max', type=float, default=0.95)
    parser.add_argument('--tgm_risk_lookback_checks', type=int, default=3)
    parser.add_argument('--tgm_tail_risk_remove_thr', type=float, default=0.5)

    parser.add_argument('--tgm_memory_enable', dest='tgm_memory_enable', action='store_true', default=True)
    parser.add_argument('--no-tgm_memory_enable', dest='tgm_memory_enable', action='store_false')
    parser.add_argument('--tgm_memory_artifact_dir', type=str, default='tail_group_memory')
    parser.add_argument('--tgm_mem_min_support_images', type=int, default=1)
    parser.add_argument('--tgm_mem_gate_quantile', type=float, default=0.9)
    parser.add_argument('--tgm_mem_gate_mad_scale', type=float, default=1.0)
    parser.add_argument('--tgm_mem_min_gate_angle', type=float, default=5.0)
    parser.add_argument('--tgm_mem_fusion_lambda', type=float, default=1.0)
    parser.add_argument('--tgm_mem_topk_ratio', type=float, default=0.05)
    parser.add_argument('--tgm_mem_chunk_size', type=int, default=4096)
    parser.add_argument('--tgm_mem_max_patches_per_class', type=int, default=20000)
