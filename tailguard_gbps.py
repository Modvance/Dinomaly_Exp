...
def _build_tail_metadata_view(tailguard_metadata_df: pd.DataFrame) -> pd.DataFrame:
    columns = ['sample_idx', 'sample_key', 'img_path', 'tail_candidate', 'pred_class_size', 'tail_score', 'group_id']
    metadata_df = tailguard_metadata_df[columns].copy()
...
def compute_global_group_risk(sample_group_scores_df: pd.DataFrame, args) -> pd.DataFrame:
    ...
    if not bool(getattr(args, 'tg_enable_global_group_risk', False)) or len(per_group_df) == 1:
    ...
    is_active = (
        delta_bic > float(getattr(args, 'tg_global_tau_bic', 0.0))
        and float(sep) > float(getattr(args, 'tg_global_tau_sep', 0.5))
        and float(conf) > float(getattr(args, 'tg_global_tau_conf', 0.1))
        and float(getattr(args, 'tg_global_pi_min', 0.05)) <= pi_high <= float(getattr(args, 'tg_global_pi_max', 0.95))
    )
...
def run_tailguard_gbps_iteration(scored_df: pd.DataFrame,
                                 ...
                                 tailguard_metadata_df: pd.DataFrame,
                                 args) -> Dict:
    ...
    metadata_view = _build_tail_metadata_view(tailguard_metadata_df)
    ...
    summary.update({
        'tailguard_local_risk_mean': float(merged_df['local_risk'].mean()) if len(merged_df) > 0 else 0.0,
        'tailguard_global_group_risk_mean': float(merged_df['global_group_risk'].mean()) if len(merged_df) > 0 else 0.0,
        'tailguard_final_risk_mean': float(merged_df['final_gbps_risk'].mean()) if len(merged_df) > 0 else 0.0,
        'tailguard_num_tail_candidates': int(merged_df['tail_candidate'].sum()),
        'tailguard_num_global_active_groups': int(global_group_metrics_df['global_is_active'].astype(bool).sum()) if len(global_group_metrics_df) > 0 else 0,
    })
    ...
    return {
        ...
        'sample_tailguard_scores_df': merged_df,
        'summary': summary,
    }
...
def aggregate_tailguard_risk_history(diag_save_dir: str,
    ...
    for name in os.listdir(diag_save_dir):
        ...
        if iter_number is None or iter_number > int(selected_iter):
            continue
        score_path = os.path.join(diag_save_dir, name, 'sample_tailguard_scores.csv')
    ...
    for iter_number in selected_iters:
        path = os.path.join(diag_save_dir, 'iter_{:05d}'.format(iter_number), 'sample_tailguard_scores.csv')
...
def build_tailguard_delete_plan(scored_df: pd.DataFrame,
                                group_assignments_df: pd.DataFrame,
                                tailguard_metadata_df: pd.DataFrame,
    ...
    risk_thr = float(getattr(args, 'tg_tail_risk_remove_thr', 0.5))
    merged_df['tailguard_remove'] = merged_df['baseline_remove']
    tail_mask = merged_df['tail_candidate'] == 1
    merged_df.loc[tail_mask, 'tailguard_remove'] = (
        merged_df.loc[tail_mask, 'baseline_remove']
        & (merged_df.loc[tail_mask, 'aggregated_final_gbps_risk'] >= risk_thr)
    )

    pruned_samples = merged_df.loc[merged_df['tailguard_remove']].copy().reset_index(drop=True)
    kept_samples = merged_df.loc[~merged_df['tailguard_remove']].copy().reset_index(drop=True)
    retained_index_map = _build_retained_index_map(kept_samples)

    tail_support_mask = (
        (merged_df['tail_candidate'] == 1)
        & (~merged_df['tailguard_remove'])
    ...
    summary = {
        ...
        'tailguard_tail_risk_remove_thr': risk_thr,
        'num_baseline_removed': int(merged_df['baseline_remove'].sum()),
        'num_tail_candidates': int((merged_df['tail_candidate'] == 1).sum()),
        'num_tail_removed': int(((merged_df['tail_candidate'] == 1) & (merged_df['tailguard_remove'])).sum()),
        'num_tail_retained': int(((merged_df['tail_candidate'] == 1) & (~merged_df['tailguard_remove'])).sum()),
    ...
    return {
        ...
        'tailguard_risk_table': merged_df,
        'summary': summary,
    }
