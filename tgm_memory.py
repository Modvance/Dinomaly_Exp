"""Memory helpers for the deprecated TGM method."""
...
def build_tail_group_memory_bank(model,
    ...
    final_bank = {}
    min_support_images = int(getattr(args, 'tgm_mem_min_support_images', 1))
    max_patches_per_group = int(getattr(
        args,
        'tgm_mem_max_patches_per_class',
        getattr(args, 'mem_max_patches_per_class', 20000),
    ))
    gate_quantile = float(getattr(args, 'tgm_mem_gate_quantile', 0.9))
    gate_mad_scale = float(getattr(args, 'tgm_mem_gate_mad_scale', 1.0))
    min_gate_angle = float(getattr(args, 'tgm_mem_min_gate_angle', 5.0))
...
def compute_tail_group_memory_scores_for_batch(model,
    ...
    chunk_size = int(getattr(args, 'tgm_mem_chunk_size', getattr(args, 'mem_chunk_size', 4096)))
    topk_ratio = float(getattr(args, 'tgm_mem_topk_ratio', getattr(args, 'mem_topk_ratio', 0.05)))
...
def run_tail_group_memory_evaluation(model,
    ...
    rows: List[dict] = []
    lambda_value = float(getattr(args, 'tgm_mem_fusion_lambda', 1.0))
