...
@torch.no_grad()
def prepare_tailguard_training_metadata(model,
                                        train_eval_dataloader,
                                        device,
                                        output_dir: Optional[str],
                                        args):
    tail_embedding_source = getattr(args, 'tg_tail_embedding_source', None) or getattr(args, 'tailsampler_embedding_source', 'encoder')
    ...
    if output_dir is not None:
        ...
        saved = save_tailguard_prepare_artifacts(
            ...
            train_metadata_df,
            metadata=metadata,
            cls_embeddings_payload=cls_embeddings_payload if bool(getattr(args, 'tg_save_cls_embeddings', False)) else None,
            analysis_details_df=None,
        )
        with open(os.path.join(output_dir, 'tailguard_group_info.json'), 'w', encoding='utf-8') as file:
