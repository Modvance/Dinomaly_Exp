"""Artifact writers for the deprecated TGM method."""
...
def save_tgm_prepare_artifacts(output_dir: str,
    ...
    candidates_path = os.path.join(output_dir, 'tgm_candidates.csv')
    group_assignments_path = os.path.join(output_dir, 'group_assignments.csv')
    train_metadata_path = os.path.join(output_dir, 'tgm_train_metadata.csv')
    summary_path = os.path.join(output_dir, 'tgm_prepare_summary.json')
    ...
    if cls_embeddings_payload is not None:
        cls_embeddings_path = os.path.join(output_dir, 'tgm_cls_embeddings.pt')
    ...
    payload = {
        ...
        'artifacts': {
            'tgm_candidates_csv': candidates_path,
            'group_assignments_csv': group_assignments_path,
            'tgm_train_metadata_csv': train_metadata_path,
            'tgm_cls_embeddings_pt': cls_embeddings_path,
    ...
    return {
        ...
        'tgm_cls_embeddings_pt': cls_embeddings_path,
        'tail_sampler_analysis_details_csv': analysis_details_path,
        'tgm_prepare_summary_json': summary_path,
    }


def save_tgm_iteration_artifacts(iter_dir: str,
                                       global_group_metrics_df: pd.DataFrame,
                                       sample_tgm_scores_df: pd.DataFrame,
                                       extra_summary: Optional[Dict] = None):
    os.makedirs(iter_dir, exist_ok=True)
    global_metrics_path = os.path.join(iter_dir, 'global_group_metrics.csv')
    sample_scores_path = os.path.join(iter_dir, 'sample_tgm_scores.csv')
    summary_path = os.path.join(iter_dir, 'summary.json')

    global_group_metrics_df.to_csv(global_metrics_path, index=False)
    sample_tgm_scores_df.to_csv(sample_scores_path, index=False)
    _update_summary_json(summary_path, extra_summary=extra_summary)

    return {
        'global_group_metrics_csv': global_metrics_path,
        'sample_tgm_scores_csv': sample_scores_path,
        'summary_json': summary_path,
    }


def save_tgm_prune_artifacts(iter_dir: str, delete_plan: Dict, extra_summary: Optional[Dict] = None):
    os.makedirs(iter_dir, exist_ok=True)
    ...
    risk_table_path = os.path.join(iter_dir, 'tgm_risk_table.csv')
    kept_path = os.path.join(iter_dir, 'tgm_kept_samples.csv')
    removed_path = os.path.join(iter_dir, 'tgm_removed_samples.csv')
    summary_path = os.path.join(iter_dir, 'tgm_prune_summary.json')

    if len(delete_plan['tail_support_samples']) > 0:
        delete_plan['tail_support_samples'].to_csv(tail_support_path, index=False)
    else:
        pd.DataFrame(columns=list(delete_plan['tgm_risk_table'].columns)).to_csv(tail_support_path, index=False)

    if len(delete_plan['tail_suspicious_samples']) > 0:
        delete_plan['tail_suspicious_samples'].to_csv(tail_suspicious_path, index=False)
    else:
        pd.DataFrame(columns=list(delete_plan['tgm_risk_table'].columns)).to_csv(tail_suspicious_path, index=False)

    delete_plan['tgm_risk_table'].to_csv(risk_table_path, index=False)
    ...
    return {
        ...
        'tgm_risk_table_csv': risk_table_path,
        'tgm_kept_samples_csv': kept_path,
        'tgm_removed_samples_csv': removed_path,
        'tgm_prune_summary_json': summary_path,
    }


def save_tgm_memory_artifacts(output_dir: str,
                                    memory_bank: Dict,
...
def save_tgm_summary(output_dir: str, summary: Dict):
    os.makedirs(output_dir, exist_ok=True)
    summary_path = os.path.join(output_dir, 'tgm_summary.json')
    with open(summary_path, 'w', encoding='utf-8') as file:
